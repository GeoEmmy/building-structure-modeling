...
st.title("🏗️ RL 기반 최적 매스 + 구조모델 자동 생성 (SAC)")
...
if st.button("🏁 최적 매스 + 구조모델 생성"):
    ...
    if mass is not None and isinstance(mass, Polygon) and mass.area > 0:
        try:
            from shapely_to_plotly import shapely_to_plotly_mesh
            fig = shapely_to_plotly_mesh(mass, height=3)
            st.plotly_chart(fig)
        except:
            st.warning("✅ mass 시각화는 생략되었으나, 구조 생성은 진행됩니다.")
        ...
        st.error("❌ 유효한 mass를 찾지 못했습니다. 정책이 setback 또는 건폐율 조건을 반복적으로 위반했을 수 있습니다.")
