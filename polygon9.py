...
def rotate_boxes(boxes, center, angle_rad):
    rot = trimesh.transformations.rotation_matrix(angle_rad, direction=[0, 0, 1], point=[center.x, center.y, 0])
...
if st.button("🔍 모델 생성"):
    ...
    side_len = int(np.sqrt(연면적 / 층수))  # ✅ side_len 계산 추가
    center = setback.centroid
    rect = setback.minimum_rotated_rectangle
    angle = get_longest_edge_angle(rect)

    offset = (center.x - side_len / 2, center.y - side_len / 2)
    boxes = auto_structure_model(
        total_area=연면적,
        num_floors=층수,
        span=스팬,
        floor_height=층고,
    ...
    rotated_boxes = rotate_boxes(boxes, center, angle)

    st.markdown(f"✅ **대지면적**: {대지면적:.1f}㎡ | **유효면적**: {유효면적:.1f}㎡")
    st.markdown(f"🏢 **건축면적**: {건축면적:.1f}㎡ | **연면적**: {연면적:.1f}㎡ | **층수**: {층수}층")

    visualize_trimesh_boxes_plotly(rotated_boxes, polygon=polygon, setback=setback)
