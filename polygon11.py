...
def auto_structure_model(total_area, num_floors, span, floor_height, offset=(0, 0), basement_floors=0):
    ...
    for b in range(basement_floors):
        ...
        boxes.append(create_box((ox + side_len / 2, oy + side_len / 2, z_base + slab_thickness / 2), slab_size))

    # 기초
    if basement_floors > 0:
        z_found = - (basement_floors) * floor_height - foundation_thickness
        boxes.append(create_box((ox + side_len / 2, oy + side_len / 2, z_found + foundation_thickness / 2),
                                (side_len, side_len, foundation_thickness)))
