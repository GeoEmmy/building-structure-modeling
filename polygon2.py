...
시도 = st.selectbox("시도 선택", 시도_options, index=시도_options.index(default_시도) if default_시도 in 시도_options else 0)

시군구_options = sorted(df[df["SIDO_NM"] == 시도]["SGG_NM"].dropna().unique())
시군구 = st.selectbox("시군구 선택", 시군구_options, index=시군구_options.index(default_시군구) if default_시군구 in 시군구_options else 0)

읍면동_options = sorted(df[(df["SIDO_NM"] == 시도) & (df["SGG_NM"] == 시군구)]["EMD_NM"].dropna().unique())
읍면동 = st.selectbox("읍면동 선택", 읍면동_options, index=읍면동_options.index(default_읍면동) if default_읍면동 in 읍면동_options else 0)
